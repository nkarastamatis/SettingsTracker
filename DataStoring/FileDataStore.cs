...
namespace SettingsTracking.DataStoring
{
    public class FileDataStore : IDataStore
    {
        ...
        public FileDataStore(System.Environment.SpecialFolder baseFolder, string filename)
            : this(ConstructPath(baseFolder, filename))
        {
        ...
        #region helper
        private static string ConstructPath(System.Environment.SpecialFolder baseFolder, string filename = null)
        {
            ...
            var settingsFileName = filename ?? "tracked_settings.xml";

            string settingsFilePath = Path.Combine(
                Environment.GetFolderPath(baseFolder),
                string.Format(@"{0}{1}{2}", companyPart, appNamePart, settingsFileName)
            );
